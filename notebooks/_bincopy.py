...
(, Intel HEX, TI-TXT and binary files).
...
class Segment:
    ...
    # fait pour la partie sans overwrite
...
class BinFile:
    ...
    opened and its data added, given that the format is Intel HEX or TI-TXT.
...
# directement dans chunked aussi quoi au final
    ...
    def add_ihex(self, records, overwrite=False):
        ...
        for record in StringIO(records):
            ...
            if type_ == IHEX_DATA:
                ...
                # merge d'intervalles caché ici !
                # un peu casse-gueule à refaire ça.
                # vérifier ici TOUTE la liste des chunks de départ

                # print(data)

                debug_parsed_records.append((address,
                                           address + size,
                                           data.copy(),
                                           self.word_size_bytes))
                ...
                raise Error(f"expected type 1..5 in record {record}, but got {type_}")
        self.debug_parsed_records = debug_parsed_records
